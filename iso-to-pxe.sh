...
mount -t iso9660 -o loop "$ISO" $CDMNT || cleanup_error

#trap cleanup_error SIGINT SIGTERM
#trap cleanup EXIT

#initramfs-vanilla   vmlinuz-vanilla
...
if [[ -f $CDMNT/boot/initramfs-vanilla ]]; then
    INITRD=initramfs-vanilla
    VMLINUZ=vmlinuz-vanilla
#else
#    INITRD=initrd.img
#    VMLINUZ=vmlinuz
fi
...
( cd "$STRIPPEDISO" && echo "$ISOBASENAME" | cpio -H newc --quiet -L -o ) |
  ...
  cat $CDMNT/boot/$INITRD - > tftpboot/$INITRD

# Kernel image.
cp $CDMNT/boot/$VMLINUZ tftpboot/$VMLINUZ
